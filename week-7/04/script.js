...
function initTexture(){
    ...
    var redtex = createTex([200,0,0,255]);
    ...
    var redtex = createTex([0,0,0,100]);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, 64, 64, 0, gl.RGBA, gl.UNSIGNED_BYTE, redtex);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.REPEAT);
...
function createTex(color) {
    ...
    for (var i = 0; i < texSize; ++i) {
        for (var j = 0; j < texSize; ++j) {
            ...
            tex[4*i*texSize+4*j] = color[0];
            tex[4*i*texSize+4*j+1] = color[1];
            tex[4*i*texSize+4*j+2] = color[2];
            tex[4*i*texSize+4*j+3] = color[3];
        }
    }
    return tex;
}
...
function render(){
    ...
    ShadowMatrix[3][3]=0; 
    ShadowMatrix[3][1]=1/(-light[1]-1.01);
