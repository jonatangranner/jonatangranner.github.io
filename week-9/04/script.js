...
var teapotScale = 1 / 4;
var teapotTransVec = vec3(0, -1.8, -3);
var dY = 0.0;
...
function main() {
    ...
    dbEye = vec3(teapotTransVec[0], 2, teapotTransVec[2]);
    dbAt = teapotTransVec;
    dbUp = vec3(0, 0, -1);
...
function render() {
    ...
    var groundCoeffs = getPlaneCoeffs(reflectViewMatrix);
    var reflectProjectionMatrix = modifyProjectionMatrix(groundCoeffs, projectionMatrix);
    var reflectVPM = mult(reflectProjectionMatrix, reflectViewMatrix);
...
function renderSolid(gl, program, obj, vpMatrix) {
    ...
    g_modelMatrix = translate(add(teapotTransVec, vec3(0, 1.8 * Math.sin(dY), 0)));
...
function getPlaneCoeffs(view) {
    var A = multvm(ground[0], view);
    var B = multvm(ground[1], view);
    var C = multvm(ground[2], view);
    var D = multvm(ground[3], view);
    var v = subtract(A, B);
    var w = subtract(C, B);
    var n = normalize(cross(w, v));

    var coeffs = vec4();
    coeffs[0] = n[0];
    coeffs[1] = n[1];
    coeffs[2] = n[2];
    coeffs[3] = (-n[0] * B[0]) - (n[1] * B[1]) - (n[2] * B[2]);
    coeffs[3] = coeffs[3] * -1;
...
function multvm(v, M) {
    ...
    for (var i = 0; i != 3; ++i)
        vM[i][i] = v[i];
    vM = mult(vM, M);
    return vec3(vM[0][0], vM[1][1], vM[2][2]);
}