(function () {
  ...
  function Draw() {
    ...
    this._gl = canvas.getContext("webgl", { alpha: false, stencil: true }) || canvas.getContext("experimental-webgl", { alpha: false, stencil: true });
    ...
    var modifiedP = (function () {
      ...
      var clipplane = vec4(nInV[0], nInV[1], nInV[2], -dot(nInV, oInV));

      return modifyProjectionMatrix(clipplane, P);
    })();

    this._normalP = P;
    this._modifiedP = modifiedP;
    ...
    this._gl.uniformMatrix4fv(this._program.texture.getUniformLocation("uP"), false, flatten(this._normalP));
    ...
    this._uFigureP = this._program.figure.getUniformLocation("uP");
    ...
    this._uShadowP = this._program.shadow.getUniformLocation("uP");
  ...
  Draw.prototype.render = function () {
    // Disable stencil test while drawing figures
    ...
    this._gl.uniformMatrix4fv(this._uShadowP, false, flatten(this._normalP));
    ...
    // 1. Update stencil buffer
    this._gl.enable(this._gl.STENCIL_TEST);
    this._gl.stencilOp(this._gl.KEEP, this._gl.KEEP, this._gl.REPLACE);
    this._gl.stencilFunc(this._gl.ALWAYS, 1, ~0);
    this._gl.colorMask(0, 0, 0, 0);

    this._gl.drawArrays(this._gl.TRIANGLES, 0, this._geometries.ground.vetricesCount);

    // 2. Update depth buffer
    this._gl.depthRange(1, 1); // always
    this._gl.depthFunc(this._gl.ALWAYS); // write the farthest depth value
    this._gl.stencilFunc(this._gl.EQUAL, 1, ~0); // match mirror’s visible pixels
    this._gl.stencilOp(this._gl.KEEP, this._gl.KEEP, this._gl.KEEP); // do not change stencil values

    this._gl.drawArrays(this._gl.TRIANGLES, 0, this._geometries.ground.vetricesCount);

    // 3a. Draw reflected figure
    this._program.figure.use();
    this._gl.depthFunc(this._gl.LESS);
    this._gl.colorMask(1, 1, 1, 1);
    this._gl.depthRange(0, 1);
    ...
    this._gl.uniformMatrix4fv(this._uFigureP, false, flatten(this._modifiedP));
    ...
    // 3b. The projection matrix breaks the depth buffer so clear the depth buffer
    // also disable the sencil test now that the reflection have been drawn
    ...
    this._gl.uniform4fv(this._uFigureLight, flatten(this._light));
    this._gl.uniformMatrix4fv(this._uFigureLightV, false, flatten(this._lightV));
    this._gl.uniformMatrix4fv(this._uFigureP, false, flatten(this._normalP));
    this._gl.uniformMatrix4fv(this._uFigureR, false, flatten(this._offR));
    this._gl.uniformMatrix4fv(this._uFigureM, false, flatten(this._figureM));

    this._gl.drawElements(this._gl.TRIANGLES, this._geometries.figure.vetricesCount, this._gl.UNSIGNED_SHORT, 0);

    // 5b. Draw reflective plane
    this._program.texture.use();
  ...
  function modifyProjectionMatrix(clipplane, projection) {
    // MV.js has no copy varructor for matrices
  ...
  function render(timestamp) {
    ...
    if (movement) draw.setFigureOffset((Math.cos(theta) * 0.5 + 0.5) * 1.8 - 0.8);
